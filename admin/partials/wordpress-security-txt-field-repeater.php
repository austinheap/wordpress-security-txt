...
<ul class="repeaters"><?php
    ...
    for ($i = 0; $i <= $count; $i ++) {
        ...
        <div class="handle">
            <span class="title-repeater"><?php echo esc_html($setatts['label-header'], 'wordpress-security-txt'); ?></span>
            <button aria-expanded="true" class="btn-edit" type="button">
                <span class="screen-reader-text"><?php echo esc_html($setatts['label-edit'],
                                                                      'wordpress-security-txt'); ?></span>
            ...
            <div class="wrap-fields"><?php
                ...
                foreach ($setatts['fields'] as $fieldcount => $field) {
                    foreach ($field as $type => $atts) {
                        ...
                        include(plugin_dir_path(__FILE__) . $this->plugin_name . '-field-' . $type . '.php'); ?></p><?php
                    }
                } // $fieldset foreach ?></div>
            <div>
                <a class="link-remove" href="#">
						<span><?php

                            echo esc_html(apply_filters($this->plugin_name . '-repeater-remove-link-label',
                                                          $setatts['label-remove']), 'wordpress-security-txt'); ?></span>
                </a>
            </div>
        </div>
        </li><!-- .repeater --><?php
