(function( $ ) {
    ...
    $(function() {
        ...
        jQuery('input[name="wordpress-security-txt-options[contact]"]').on('click keyup keypress blur change focus', function () {
            window.WORDPRESS_SECURITY_TXT_VALIDATORS.stylize(
                jQuery(this),
                window.WORDPRESS_SECURITY_TXT_VALIDATORS.contact(jQuery(this).val())
            );
        }).trigger('click');
        ...
        jQuery('textarea[name="wordpress-security-txt-options[encryption]"]').on('click keyup keypress blur change focus', function () {
            window.WORDPRESS_SECURITY_TXT_VALIDATORS.stylize(
                jQuery(this),
                window.WORDPRESS_SECURITY_TXT_VALIDATORS.encryption(jQuery(this).val())
            );
        }).trigger('click');
        ...
        jQuery('input[name="wordpress-security-txt-options[acknowledgement]"]').on('click keyup keypress blur change focus', function () {
            window.WORDPRESS_SECURITY_TXT_VALIDATORS.stylize(
                jQuery(this),
                window.WORDPRESS_SECURITY_TXT_VALIDATORS.acknowledgement(jQuery(this).val())
            );
        }).trigger('click');
